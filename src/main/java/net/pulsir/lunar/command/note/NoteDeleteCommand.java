...
public class NoteDeleteCommand extends Command {
            ...
            if (args.length == 2) {
                ...
                if (!Lunar.getInstance().getNoteManager().noteExists(offlinePlayer.getUniqueId(), note)) {
                    player.sendMessage(Lunar.getInstance().getMessage().getMessage(Objects.requireNonNull(Lunar.getInstance()
                                    .getLanguage().getConfiguration().getString("NOTES.NOT-EXISTS"))
                            .replace("{player}", args[1])
                            .replace("{note}", note)));
                    return;
                }

                Lunar.getInstance().getNoteManager().deleteNote(offlinePlayer.getUniqueId(), note);
                player.sendMessage(Lunar.getInstance().getMessage().getMessage(Objects.requireNonNull(Lunar.getInstance()
                                .getLanguage().getConfiguration().getString("NOTES.DELETED"))
                        .replace("{player}", args[1])));
            }
        }
    }
    ...
    public Component getUsage() {
        return Lunar.getInstance().getMessage().getMessage(Lunar.getInstance()
                .getLanguage().getConfiguration().getString("NOTES.USAGE.DELETE"));
    }
    ...
    public String permission() {
        return "lunar.command.note.delete";
    }
}
