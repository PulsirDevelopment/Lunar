...
public class MaintenanceListener implements Listener {
    ...
    public void onLogin(PlayerLoginEvent event) {
        if (Lunar.getInstance().getServerMaintenanceManager().isMaintenanceActive() &&
            ...
            event.disallow(PlayerLoginEvent.Result.KICK_WHITELIST, generateMaintenanceMessage());
    }

    private Component generateMaintenanceMessage() {
        ...
        for (String line : Lunar.getInstance().getLanguage().getConfiguration().getStringList("MAINTENANCES.SERVER-ENTRY")) {
            message.append(Lunar.getInstance().getMessage().getMessage(line
                    .replace("{name}", activeMaintenance.getName())
                    .replace("{reason}", activeMaintenance.getReason())));
        }
